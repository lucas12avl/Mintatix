...
const ChainSwitcher = ({targetChainId }: { chainId?: number, targetChainId?: number }) => {

  const account = useAccount() //get the address of the user
  const { chains, switchChain } = useSwitchChain()

  if (!chains || account.chainId === targetChainId) return null; //if the chainId is undefined or the user is in the right chain, then don't show anything
