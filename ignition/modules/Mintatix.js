...
module.exports = buildModule("Mintatix", (m) => {
  ...
  const EVENT_USE_TIME_LIMIT = [true, true, true, true] //if the event has a time limit
  ...
  for (let i = 0; i < numProxies; i++) { //create events
    ...
    const createEventTx = m.call(factory, "createEvent",[EVENT_NAME[i], EVENT_SYMBOL[i], EVENT_TICKET_PRICE, EVENT_MAX_SUPPLY, EVENT_URI[i], EVENT_BASE_TOKEN_URI[i], EVENT_MAX_TICKETS_PER_ADDRESS, EVENT_USE_TIME_LIMIT[i], START_PURCHASE_TIME[i], EVENT_END_TIME[i]], {id: "createEvent_" + EVENT_SYMBOL[i]});
